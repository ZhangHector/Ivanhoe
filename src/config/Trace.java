 ...
 * @Author Hector Zhang
 * @Version 1.1
 * @Referecnce Howard Scott Needham
 * @Version 1.0
 * 
...
public class Trace {
	...
	public void write(Object o, String m, int i) {
		String message = String.format("%s %d", m ,i);
		write(o,message);
	}
	
	public void write(Object o, String m, long l) {
		String message = String.format("%s %s", m , Long.toString(l));
		write(o,message);
	}
	
	public void write(Object o, String m, Socket socket) {
		String message = String.format("%s : Client Address : [%15s] Client Socket: [%-6d]", m, socket.getRemoteSocketAddress(), socket.getPort());
		write (o,message);
	}

	public void exception(Exception e) {
		String message = String.format("Exception thrown : %s \n", e.getMessage());
		if (LANConfig.PRINT_STACK_TRACE) e.printStackTrace();
		write(this,message);	
	}
	...
	public void write(Object o, String m, ServerSocket server) {
		String message = format (m,String.format("%s:%s", server.getInetAddress(), server.getLocalPort()));
		write (o,message);
	}
	...
	public void toServer (Object o, ServerThread from, ServerSocket server, String m) {
		String message = String.format("Sending From [%12s:%d] To [%12s%d] : %s", from.getSocketAddress(), from.getID(), server.getInetAddress(), server.getLocalPort(), m);
		writeToClientFile (o,message);
	}
	...
	private String format (Object o, String message) {
		return String.format ("[Time: %23s] Class: %-12s: %s\n",getDateTime(),o.getClass().getSimpleName(), message) ;
	}
